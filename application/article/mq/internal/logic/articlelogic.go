...
import (
	...
	"github.com/LinCrayon/beyond/application/article/mq/internal/types"
...
func (l *ArticleLogic) Consume(_, val string) error {
	...
	err := json.Unmarshal([]byte(val), &msg) // JSON 字符串 val 反序列化为 msg 对象
	if err != nil {
...
func (l *ArticleLogic) articleOperate(msg *types.CanalArticleMsg) error {
	...
	for _, d := range msg.Data {
		...
		case types.ArticleStatusVisible:
			//判断publishTimeKey的有序集合（sorted set）是否存在，存在就添加数据
